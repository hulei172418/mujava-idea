...
import static mujava.op.basic.MethodLevelMutator.*;
...
public class AllMutantsGenerator extends MutantsGenerator
{
  ...
  void getMethods(ClassDeclaration classDecl) throws Exception {
     MutationSystem.WHOLE_METHOD_NAMES.clear();
      // 获取所有的方法声明
      if (classDecl != null) {
         MemberDeclarationList methods = classDecl.getBody();
         for (int i = 0; i < methods.size(); i++) {
            MemberDeclaration member = methods.get(i);

            // 只处理方法
            if (member instanceof MethodDeclaration) {
               MethodDeclaration method = (MethodDeclaration) member;
               String tmp = getMethodSignature(method);
               // System.out.println(tmp);
               MutationSystem.WHOLE_METHOD_NAMES.add(tmp);
            }else if (member instanceof ConstructorDeclaration){
               ConstructorDeclaration method = (ConstructorDeclaration) member;
               String tmp = getConstructorSignature(method);
               // System.out.println(tmp);
               MutationSystem.WHOLE_METHOD_NAMES.add(tmp);
            }
         }
   ...
   void genTraditionalMutants(ClassDeclarationList cdecls)
   {
      ...
      {
         ClassDeclaration cdecl = cdecls.get(j);
         MutationSystem.CLASS_NAME_T = cdecl.getName();
