...
//      
   ...
   private void showEmptyResult(JTable table, JList killed_list, JList live_list)
   {
      ...
      resultModel.setValueAt( "  " + Integer.toString(0) , 0 , 1 );   // live mutant
      resultModel.setValueAt( "  " + Integer.toString(0) , 1 , 1 );   // killed mutant
      resultModel.setValueAt( "  " + Integer.toString(0) , 2 , 1 );   // total
   ...
   { 
      ...
      float mutant_score = (float) ((killed_num * 100) / (killed_num + live_num));

      // Show the result on resultTable
      ResultTableModel resultModel = (ResultTableModel)(table.getModel());
      resultModel.setValueAt( "  " + Integer.toString(live_num) , 0 , 1 );   // live mutant
      resultModel.setValueAt( "  " + Integer.toString(killed_num) , 1 , 1 ); // killed mutant
      resultModel.setValueAt( "  " + Integer.toString(live_num+killed_num) , 2 , 1 );   //total
      resultModel.setValueAt( "  " + Float.toString(mutant_score) + "%" , 3 , 1 );   // mutant score
   ...
   void showGeneratedClassMutantsNum(String[] name)
   {
      if (name != null)
      {
         ...
         for (int i=0; i<MutationSystem.cm_operators.length; i++)
         {
            myModel.setValueAt(Integer.toString(num[i]), i, 1);
            total = total + num[i];
         }
