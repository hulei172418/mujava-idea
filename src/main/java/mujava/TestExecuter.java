...
public class TestExecuter {
    ...
    public TestResult runTraditionalMutants(String methodSignature)  throws NoMutantException,NoMutantDirException{
        ...
        if(methodSignature.equals("All method")){
            try{
                ...
                File f_kill = new File(MutationSystem.TRADITIONAL_MUTANT_PATH, "kill_statistic.txt");
                FileOutputStream fout = new FileOutputStream(f_kill);
                ...
                out.println("test_results=" + finalTestResults +
                        ";\nmutant_results=" + finalMutantResults +
    ...
    private TestResult runMutants(TestResult tr, String methodSignature)
        ...
        try {
            ...
            int mutant_num = (mutantDirectories == null) ? 0 : mutantDirectories.length;
            ...
            for (int i = 0; i < tr.mutants.size(); i++) {
                final String mutant_name = tr.mutants.get(i).toString();
                String info = mutant_name+" ("+i+"th)"+tr.mutants.size();
                ...
                // 1) 固化测试类（避免成员变量被并发覆盖）
                final Class<?> execClass = mutantLoader.loadTestClass(testSet);
                if (execClass == null) {
                    System.err.println("Cannot load test class: " + testSet + " for mutant " + mutant_name);
                    // 你可以选择把它当作“不可执行”，这里先记到报告里
                    finalMutantResults.put(mutant_name, "cannot_load_test_class");
                    tr.live_mutants.add(mutant_name);
                    continue;
                }

                // 2) 每个 mutant 一份局部结果表：线程启动前就初始化，超时分支永远安全
                final Map<String, String> mutantResultsLocal = new HashMap<>();
                for (String tname : junitTests) {
                    mutantResultsLocal.put(tname, "pass");
                }

                // 3) 每个 mutant 独立锁 + 独立完成标志，避免共享 lockObject/mutantRunning 的竞态
                ...
                Thread t = new Thread(() -> {
                    try {
                        JUnitCore jCore = new JUnitCore();
                        Result res = jCore.run(execClass);

                        List<Failure> listOfFailure = res.getFailures();
                        for (Failure failure : listOfFailure) {
                            String nameOfTest = failure.getTestHeader()
                                    .substring(0, failure.getTestHeader().indexOf("("));
                            String testSourceName = testSet + "." + nameOfTest;

                            String[] sb = failure.getTrace().split("\\n");
                            String lineNumber = "";
                            for (int x = 0; x < sb.length; x++) {
                                if (sb[x].indexOf(testSourceName) != -1) {
                                    lineNumber = sb[x].substring(sb[x].indexOf(":") + 1, sb[x].indexOf(")"));
                                }
                            }

                            if (failure.getMessage() == null || failure.getMessage().equals("")) {
                                mutantResultsLocal.put(nameOfTest, nameOfTest + ": " + lineNumber + "; " + "fail");
                            } else {
                                mutantResultsLocal.put(nameOfTest, nameOfTest + ": " + lineNumber + "; " + failure.getMessage());
                            }
                        }

                        System.out.println(mutantResultsLocal.toString());

                    } catch (Throwable e) {
                        // 不要吞异常，否则主线程会误以为“还在跑”-> 一直 timeout
                        e.printStackTrace();
                    } finally {
                        finished.set(true);
                ...
                synchronized (localLock) {
                    localLock.wait(TIMEOUT);
                }

                // 4) 超时分支：此时 mutantResultsLocal 已经初始化过，直接填 timeout
                if (!finished.get()) {
                    t.interrupt(); // 注意：interrupt 不能保证真的停掉，但至少不会 NPE/串写
                    // System.out.println(" time_out: more than " + TIMEOUT + " milliseconds");

                    for (String tname : junitTests) {
                        mutantResultsLocal.put(tname, "time_out: more than " + TIMEOUT + " milliseconds");
                    }

                    // 可选：给一点点时间让线程自行结束，减少后续堆积（不保证有效）
                    try { t.join(50); } catch (InterruptedException ignored) {}
                }

                // 5) 用 mutantResultsLocal 和 originalResults 比较，空安全
                boolean killed = false;
                for (int k = 0; k < junitTests.size(); k++) {
                    String name = junitTests.get(k);

                    if (!java.util.Objects.equals(mutantResultsLocal.get(name), originalResults.get(name))) {
                        killed = true;

                        String prevByTest = finalTestResults.getOrDefault(name, "");
                        if (prevByTest.equals("")) finalTestResults.put(name, mutant_name);
                        else finalTestResults.put(name, prevByTest + ", " + mutant_name);

                        String prevByMut = finalMutantResults.getOrDefault(mutant_name, "");
                        if (prevByMut.equals("")) finalMutantResults.put(mutant_name, name);
                        else finalMutantResults.put(mutant_name, prevByMut + ", " + name);
                    }
                }

                if (killed) tr.killed_mutants.add(mutant_name);
                else tr.live_mutants.add(mutant_name);

                mutantLoader = null;
                // 不建议每轮 System.gc(); 反而可能拖慢/抖动
            }

            // 移除 killed from live（你原逻辑保留）
    ...
    private TestResult runMutants1(TestResult tr, String methodSignature) throws NoMutantException,NoMutantDirException{
        try{
            ...
            for(int i = 0; i < tr.mutants.size(); i++){
                ...
                for(int k = 0;k < junitTests.size();k++){
                    String name = junitTests.get(k);
                    if (!java.util.Objects.equals(mutantResults.get(name), originalResults.get(name))) {
                        sign = true;
