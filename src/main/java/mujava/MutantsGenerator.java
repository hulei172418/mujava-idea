...
//	static int  counter;
  ...
  /*void generateMutant(OJClass mutant_op){
   ...
   public void compileMutants()
   {
      ...
      {
         String fi = MutationSystem.MUTANT_PATH + "/" + s[i];
         File target_dir = new File(fi);
         String[] files = target_dir.list();
         ArrayList<String> target_file = new ArrayList<>();
         for (int j = 0; j < Objects.requireNonNull(files).length; j++) {
            String fj = fi + "/" + files[j];
            File tmp_file = new File(fj);
            String[] tmp_files = tmp_file.list(new ExtensionFilter("java"));
            assert tmp_files != null;
            for (int k = 0; k < Objects.requireNonNull(tmp_files).length; k++) {
               String fk = fj + "/" + tmp_files[k];
               target_file.add(fk);
            }
         }

         String[] pars = null;
         for (String value : target_file) {
            pars = new String[3];
            pars[0] = "-classpath";
            pars[1] = MutationSystem.JAR_PATH;
            pars[2] = value;
            try {
               // result = 0 : SUCCESS,   result = 1 : FALSE
               int result = Main.compile(pars, new PrintWriter(System.out));
               if (result == 0) {
                  Debug.println(value + " Compile Finished");
                  counter++;
               } else {
                  File rm_f = new File(value);
                  if (!rm_f.exists())
                     continue;
                  boolean b = rm_f.delete();
                  if(!b){
                     Debug.println(value + " Delete Failed");
                  }
                  b = new File(rm_f.getParent()).delete();
                  if(!b){
                     Debug.println(rm_f.getParent() + " Delete Failed");
                  }
//                  CodeChangeLog.deleteLine("AOIS_1:16:byte_fill(byte,byte):val => val++");
               }
            } catch (Exception e) {
